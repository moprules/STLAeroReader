...
const Vector = {
    ...
    */
    ...
    heronArea: function (A, B, C) {
        const AB = Vector.absV(Vector.vectSubt(A, B))
        const BC = Vector.absV(Vector.vectSubt(B, C))
        const CA = Vector.absV(Vector.vectSubt(C, A))

        const p = 0.5 * (AB + BC + CA)

        let res = Math.sqrt(p * (p - AB) * (p - BC) * (p - CA))
        if (isNaN(res))
        {
            res = 0
        }
    ...
    solve: function (A, b) {
        ...
        if (D != 0) {
            for (let i = 0; i < 2; i++) {
                let M = this.replaceCol(A, b, i)
                let detM = this.det2(M)
                res[i] = detM / D
            }
        }
